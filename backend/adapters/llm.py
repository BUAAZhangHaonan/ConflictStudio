...
class UnconfiguredPromptModel:
    ...
    async def generate(self, system_input: str, user_input: str) -> str:
        raise PromptAdapterError(
            ...
            "Prompt generation requires CONFLICTSTUDIO_LLM_BASE_URL and CONFLICTSTUDIO_LLM_API_KEY",
        )
...
class OpenAICompatiblePromptModel:
    ...
    def __init__(self, base_url: str, api_key: str, client: httpx.AsyncClient | None = None) -> None:
        if not base_url.strip() or not api_key.strip():
            raise ValueError("Prompt service base URL and API key are required")
    ...
    def from_environment(cls) -> PromptModel:
        base_url = os.environ.get("CONFLICTSTUDIO_LLM_BASE_URL", "").strip()
