...
def test_classification_history_is_append_only_and_keeps_old_reviews(
    tmp_path: Path,
) -> None:
    app = sample_app(tmp_path)
    with TestClient(app) as client:
        ...
        history = client.get(
            f"/api/samples/{sample['id']}/classification-history"
        )
        reviews = client.get("/api/reviews", params={"sampleId": sample["id"]})

        direction_filter = client.get(
            "/api/samples",
            params={"relation": "Conflict", "direction": "Audio"},
        )
        surface_search = client.get(
            "/api/samples",
            params={"search": "tense"},
        )
    assert moved.status_code == 200
    ...
    assert direction_filter.json()["total"] == 1
    assert surface_search.json()["total"] == 1
    record = history.json()["items"][0]
    assert record["operatorId"] == reviewer["id"]
    assert record["operatorName"] == reviewer["name"]
    assert record["beforeProtocol"] == record["afterProtocol"] == "VA"
    assert record["beforeRelation"] == "Aligned"
    assert record["afterRelation"] == "Conflict"
    assert record["beforeDirection"] is None
    assert record["afterDirection"] == "Audio"
    assert record["beforeSampleRevision"] == reviewed["revision"]
    assert record["afterSampleRevision"] == moved.json()["revision"]
    ...
    try:
        with pytest.raises(sqlite3.IntegrityError):
            connection.execute(
                "UPDATE sample_classification_changes "
                "SET after_apparent_emotion = 'changed' WHERE id = ?",
                (record["id"],),
            )
        with pytest.raises(sqlite3.IntegrityError):
            connection.execute(
                "DELETE FROM sample_classification_changes WHERE id = ?",
                (record["id"],),
            )
    finally:
        connection.close()
