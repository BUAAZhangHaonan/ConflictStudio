      ...
      <div className="generation-layout">
        <section className="panel generation-form" aria-label={g('batches.formRegion')}>
          ...
          <div className="generation-form__grid">
            ...
            <fieldset className="generation-form__wide generation-fieldset"><legend>{g('batches.backgrounds')}</legend><div className="generation-choice-grid">{activeBackgrounds.map(item => <label key={item.id}><input type="checkbox" checked={form.backgroundPresetIds.includes(item.id)} onChange={() => setForm(current => ({ ...current, backgroundPresetIds: toggleArrayValue(current.backgroundPresetIds, item.id) }))} /><span>{localizedName(locale, item)}</span></label>)}</div></fieldset>
          </div>
