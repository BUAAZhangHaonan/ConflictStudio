      ...
      <div className="generation-layout">
        <section className="panel generation-form" aria-label={g('batches.formRegion')}>
          ...
          <div className="generation-form__grid">
            ...
            <Field className="generation-batch-category" label={g('batches.category')} htmlFor="batch-category" required><select id="batch-category" value={form.category} onChange={event => changeCategory(event.target.value as Category)}>{categories.map(value => <option key={value} value={value}>{categoryLabel(g, value)}</option>)}</select></Field>
            ...
            <fieldset className="generation-form__wide generation-fieldset"><legend>{g('batches.backgrounds')}</legend><div className="generation-fieldset__toolbar"><Button type="button" variant="quiet" disabled={activeBackgrounds.length === 0 || activeBackgrounds.every(item => form.backgroundPresetIds.includes(item.id))} onClick={() => setForm(current => ({ ...current, backgroundPresetIds: activeBackgrounds.map(item => item.id) }))}>{g('batches.selectAllBackgrounds')}</Button></div><div className="generation-choice-grid">{activeBackgrounds.map(item => <label key={item.id}><input type="checkbox" checked={form.backgroundPresetIds.includes(item.id)} onChange={() => setForm(current => ({ ...current, backgroundPresetIds: toggleArrayValue(current.backgroundPresetIds, item.id) }))} /><span>{localizedName(locale, item)}</span></label>)}</div></fieldset>
          </div>
          ...
          <div className="generation-form__actions generation-form__actions--with-status">
            <p className="generation-unsaved-status" role="status" aria-live="polite" aria-atomic="true">{dirty ? g('batches.unsavedStatus') : ''}</p>
            <Button onClick={() => void save()} disabled={!dirty || saveMutation.isPending}>{g('batches.saveDraft')}</Button>
            <Button variant="primary" onClick={() => void buildPreview()} disabled={!isSavedCurrent || previewMutation.isPending}>{g('batches.preview')}</Button>
          </div>
        </section>
        <GpuPanel />
      </div>
