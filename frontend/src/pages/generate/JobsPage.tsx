      ...
      <div className={`generation-layout generation-layout--jobs generation-jobs${params.has('job') ? ' generation-layout--job-detail' : ''}`}>
        ...
        <section className="panel generation-job-detail" aria-label={g('jobs.detailRegion')}>
          ...
          {!selected ? <p className="generation-empty-note">{g('jobs.empty')}</p> : <>
            ...
            <section className="generation-job-section" aria-labelledby="job-items-title"><h3 id="job-items-title">{g('jobs.items')}</h3>{items.length === 0 ? <p>{g('jobs.noCurrentInput')}</p> : <div className="generation-result-cards">{items.map(item => <article className="generation-result-card" key={item.id}><div className="generation-result-card__header"><div><h3>{item.sequence}/{selected.totalCount}</h3><p>{item.input.model}{item.input.precision ? ` ${item.input.precision}` : ''} / {item.gpuSlot}</p></div><StatusBadge label={g(`jobs.status.${item.status}`)} kind={jobStatusKind(item.status)} /></div><div className="generation-result-card__asset">{item.primaryAssetUrl ? <video controls muted={item.input.deriveSilentPrimary} src={item.primaryAssetUrl}>{g('test.videoUnsupported')}</video> : <span>{g('test.mediaPlaceholder')}</span>}</div>{item.sourceAssetUrl ? <a href={item.sourceAssetUrl}>{g('jobs.audio')}</a> : null}<dl className="generation-current-input"><div><dt>{g('jobs.person')}</dt><dd>{g(`demographic.age.${item.input.age}`)} / {g(`demographic.gender.${item.input.gender}`)} / {g(`demographic.ethnicity.${item.input.ethnicity}`)}</dd></div><div><dt>{g('jobs.seed')}</dt><dd>{item.input.seed}</dd></div>{item.failureCode ? <div><dt>{g('jobs.failureTitle')}</dt><dd>{jobFailureMessage(item.failureCode, g)}</dd></div> : null}<div className="generation-current-input__prompt"><dt>{g('promptPreview.positive')}</dt><dd><pre>{item.promptResult?.finalPositivePrompt ?? item.input.fixedPositivePrompt ?? item.input.userInput}</pre></dd></div><div className="generation-current-input__prompt"><dt>{g('promptPreview.negative')}</dt><dd><pre>{item.promptResult?.finalNegativePrompt ?? item.input.finalNegativePrompt}</pre></dd></div></dl><details className="generation-attempts"><summary>{g('jobs.attemptHistory')}</summary>{item.attempts.length === 0 ? <p>{g('jobs.noResults')}</p> : <ol className="generation-attempt-list">{item.attempts.map(attempt => <li key={attempt.id}><div><strong>{g('jobs.attempt', { number: attempt.attemptNumber })}</strong><span>{attempt.model}{attempt.precision ? ` ${attempt.precision}` : ''} / {attempt.gpuSlot} / {g(`jobs.status.${attempt.status}`)}</span>{attempt.failureReason ? <span>{jobFailureMessage(item.failureCode, g)}</span> : null}</div><div>{attempt.primaryAssetUrl ? <a href={attempt.primaryAssetUrl}>{g('jobs.openAttempt', { number: attempt.attemptNumber })}</a> : null}{attempt.sourceAssetUrl ? <a href={attempt.sourceAssetUrl}>{g('jobs.audio')}</a> : null}</div></li>)}</ol>}</details>{selected.source === 'Test' && item.status === 'Completed' && item.primaryAssetId !== null ? <div className="generation-result-card__actions"><Button variant="primary" disabled={item.sampleId !== null || productionDatasets.length === 0} onClick={() => openKeep(item)}>{item.sampleId === null ? g('jobs.keep') : g('jobs.kept')}</Button></div> : null}</article>)}</div>}</section>
