...
export function JobsPage() {
  ...
  const [attemptItemId, setAttemptItemId] = useState<number | null>(null);
  ...
  const jobsQuery = useJobsQuery(jobPage);
  const jobQuery = useJobQuery(selectedId);
  const itemsQuery = useJobItemsQuery(selectedId, itemPage);
  const eventsQuery = useJobEventsQuery(selectedId, eventPage);
  const attemptsQuery = useJobAttemptsQuery(attemptItemId, attemptPage);
  ...
  const selected = jobQuery.data ?? null;
  ...
  useEffect(() => {
    if (selectedId === null && jobs.length > 0) setSelectedId(jobs[0].id);
  }, [jobs, selectedId]);
  ...
  const productionDatasets = (datasetsQuery.data?.items ?? []).filter(dataset => dataset.status === 'Active' && dataset.purpose === 'Formal');
  ...
  const showLatestLogs = async () => {
    const result = await eventsQuery.refetch();
    setEventPage(result.data?.totalPages ?? 1);
    replay.clearNewEvents();
  };

  if (jobsQuery.isPending) {
    return <GenerationScaffold title="jobs.title" subtitle="jobs.subtitle"><p role="status">{g('state.loadingBody')}</p></GenerationScaffold>;
  }
  if (jobsQuery.isError) {
    return <GenerationScaffold title="jobs.title" subtitle="jobs.subtitle"><OperationFeedback error={jobsQuery.error} onDismiss={() => void jobsQuery.refetch()} /></GenerationScaffold>;
  }

  const detailError = jobQuery.error ?? itemsQuery.error ?? eventsQuery.error ?? attemptsQuery.error ?? gpuQuery.error ?? datasetsQuery.error ?? cancelMutation.error ?? keepMutation.error ?? null;
  return (
    <GenerationScaffold title="jobs.title" subtitle="jobs.subtitle">
      {detailError ? <OperationFeedback error={detailError} onDismiss={() => { cancelMutation.reset(); keepMutation.reset(); void Promise.all([jobQuery.refetch(), itemsQuery.refetch(), eventsQuery.refetch(), attemptsQuery.refetch(), gpuQuery.refetch(), datasetsQuery.refetch()]); }} /> : null}
      ...
      <div className={`generation-layout generation-layout--jobs generation-jobs${params.has('job') ? ' generation-layout--job-detail' : ''}`}>
        <section className="panel generation-list" aria-labelledby="jobs-list-title">
          <div className="section-header"><h2 id="jobs-list-title">{g('jobs.list')}</h2><span aria-live="polite">{g('jobs.count', { count: jobsQuery.data?.total ?? 0 })}</span></div>
          ...
          {filtered.length === 0 ? <p className="generation-empty-note">{g(jobs.length === 0 ? 'jobs.empty' : 'jobs.filtered')}</p> : <ul className="generation-job-list" aria-label={g('jobs.tableCaption')}>{filtered.map(job => {
          ...
          <Pagination page={jobsQuery.data?.page ?? 1} totalPages={jobsQuery.data?.totalPages ?? 0} total={jobsQuery.data?.total ?? 0} onPageChange={setJobPage} />
        </section>
        <section className="panel generation-job-detail" aria-label={g('jobs.detailRegion')}>
          <Button className="generation-job-back" variant="quiet" onClick={backToList}>{g('jobs.backToList')}</Button>
          {!selected ? <p className="generation-empty-note">{g('jobs.empty')}</p> : <>
            ...
            {selected.failureCode ? <section className="generation-job-failure" role="alert"><h3>{g('jobs.failureTitle')}</h3><p>{jobFailureMessage(selected.failureCode, g)}</p></section> : null}
            ...
            <section className="generation-job-section" aria-labelledby="job-items-title">
              ...
              {items.length === 0 ? <p>{g('jobs.noCurrentInput')}</p> : <div className="generation-result-cards">{items.map(item => <article className="generation-result-card" key={item.id}>
              ...
              <Pagination page={itemsQuery.data?.page ?? 1} totalPages={itemsQuery.data?.totalPages ?? 0} total={itemsQuery.data?.total ?? 0} onPageChange={setItemPage} />
            </section>
            {attemptItemId !== null ? <section className="generation-job-section" aria-labelledby="job-attempts-title"><div className="section-header"><h3 id="job-attempts-title">{g('jobs.attemptHistory')}</h3><Button variant="quiet" onClick={() => setAttemptItemId(null)}>{g('common.close')}</Button></div>{attempts.length === 0 ? <p>{g('jobs.noResults')}</p> : <ol className="generation-attempt-list">{attempts.map(attempt => <li key={attempt.id}><div><strong>{g('jobs.attempt', { number: attempt.attemptNumber })}</strong><span>{attempt.model}{attempt.precision ? ` ${attempt.precision}` : ''} / {attempt.gpuSlot} / {g(`jobs.status.${attempt.status}`)}</span>{attempt.failureReason ? <span>{g('jobs.failure.general')}</span> : null}</div><div>{attempt.primaryAssetUrl ? <a href={attempt.primaryAssetUrl}>{g('jobs.openAttempt', { number: attempt.attemptNumber })}</a> : null}{attempt.sourceAssetUrl ? <a href={attempt.sourceAssetUrl}>{g('jobs.audio')}</a> : null}</div></li>)}</ol>}<Pagination page={attemptsQuery.data?.page ?? 1} totalPages={attemptsQuery.data?.totalPages ?? 0} total={attemptsQuery.data?.total ?? 0} onPageChange={setAttemptPage} /></section> : null}
            <section className="generation-job-section" aria-labelledby="job-events-title"><div className="section-header"><h3 id="job-events-title">{g('jobs.events')}</h3>{replay.newEventCount > 0 ? <Button variant="quiet" onClick={() => void showLatestLogs()}>{g('jobs.showNewLogs', { count: replay.newEventCount })}</Button> : null}</div>{events.length === 0 ? <p>{g('jobs.noLogs')}</p> : <ol className="generation-log-list">{events.map(event => <li key={event.id}><span>{g(`jobs.event.${event.eventType}`)}</span><time dateTime={event.createdAt}>{formatDateTime(event.createdAt)}</time></li>)}</ol>}<Pagination page={eventsQuery.data?.page ?? 1} totalPages={eventsQuery.data?.totalPages ?? 0} total={eventsQuery.data?.total ?? 0} onPageChange={page => { setEventPage(page); replay.clearNewEvents(); }} /></section>
          </>}
        </section>
      </div>
