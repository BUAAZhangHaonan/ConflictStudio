    ...
    <GenerationScaffold title="jobs.title" subtitle="jobs.subtitle">
      ...
      <div className="generation-layout generation-layout--jobs generation-jobs">
        <section className="panel generation-list" aria-labelledby="jobs-list-title">
          ...
          {filtered.length === 0 ? <p className="generation-empty-note">{g(jobs.length === 0 ? 'jobs.empty' : 'jobs.filtered')}</p> : <ul className="generation-job-list" aria-label={g('jobs.tableCaption')}>{filtered.map(job => <li key={job.id}><button type="button" className={job.id === selectedId ? 'generation-job-row is-selected' : 'generation-job-row'} aria-pressed={job.id === selectedId} onClick={() => choose(job.id)}><span className="generation-job-row__header"><strong>{job.displayName}</strong><StatusBadge label={g(`jobs.status.${job.status}`)} kind={jobStatusKind(job.status)} /></span><span className="generation-job-row__meta">{categoryLabel(g, job.category)} / {job.model}</span><span className="generation-job-row__progress">{job.completedCount + job.failedCount}/{job.totalCount} / {progress(job.completedCount, job.failedCount, job.totalCount)}%</span></button></li>)}</ul>}
        </section>
        <section className="panel generation-job-detail" aria-label={g('jobs.detailRegion')}>
          {!selected ? <p className="generation-empty-note">{g('jobs.empty')}</p> : <>
            ...
            <section className="generation-job-progress" aria-labelledby="job-progress-title"><h3 id="job-progress-title">{g('jobs.progress')}</h3><progress value={selected.completedCount + selected.failedCount} max={selected.totalCount} aria-label={g('jobs.countProgressLabel', { completed: selected.completedCount + selected.failedCount, total: selected.totalCount })} /><p>{selected.completedCount + selected.failedCount}/{selected.totalCount}</p></section>
            ...
            {selected.status === 'Failed' ? <section className="generation-job-failure" role="alert"><h3>{g('jobs.failureTitle')}</h3><p>{g('jobs.failureSafe')}</p></section> : null}
            ...
            <section className="generation-job-section" aria-labelledby="job-items-title"><h3 id="job-items-title">{g('jobs.items')}</h3>{items.length === 0 ? <p>{g('jobs.noCurrentInput')}</p> : <ol className="generation-item-list">{items.map(item => <li key={item.id}><details><summary>{item.sequence}/{selected.totalCount} / {g(`jobs.status.${item.status}`)} / {item.gpuSlot}</summary><dl className="generation-current-input"><div><dt>{g('jobs.person')}</dt><dd>{item.input.age} / {item.input.gender} / {item.input.ethnicity}</dd></div><div><dt>{g('jobs.seed')}</dt><dd>{item.input.seed}</dd></div><div><dt>{g('promptPreview.positive')}</dt><dd><pre>{item.promptResult?.finalPositivePrompt ?? item.input.fixedPositivePrompt ?? item.input.userInput}</pre></dd></div><div><dt>{g('promptPreview.negative')}</dt><dd><pre>{item.promptResult?.finalNegativePrompt ?? item.input.finalNegativePrompt}</pre></dd></div></dl></details></li>)}</ol>}</section>
