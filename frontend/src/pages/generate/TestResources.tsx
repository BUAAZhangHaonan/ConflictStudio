    ...
    <section className="generation-resources" aria-labelledby="test-resources-title">
      ...
      <div className="generation-resource-grid">
        ...
        <section className="panel generation-resource-editor" aria-labelledby="version-resource-title">
          ...
          <Field label={g('test.template')} htmlFor="resource-template-select"><select id="resource-template-select" value={selectedTemplateId ?? ''} onChange={event => { setSelectedTemplateId(event.target.value ? Number(event.target.value) : null); setSelectedVersionId(null); setVersionPage(1); }}><option value="">{g('common.none')}</option>{templates.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>
          <Field label={g('test.resource.existingVersion')} htmlFor="resource-version-select"><select id="resource-version-select" value={selectedVersionId ?? ''} onChange={event => setSelectedVersionId(event.target.value ? Number(event.target.value) : null)}><option value="">{g('common.none')}</option>{versions.map(item => <option key={item.id} value={item.id}>{item.templateName} {item.version}</option>)}</select></Field>
