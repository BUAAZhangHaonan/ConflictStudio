...
export function WorkspacePage() {
  ...
  const error = datasetsQuery.error ?? jobsQuery.error ?? createMutation.error ?? updateMutation.error ?? null;
  const pendingReview = snapshot.data.samples.filter(sample => sample.reviewDecision === 'Pending').length;
  const archivedIds = new Set(snapshot.data.archives.flatMap(archive => archive.currentSampleIds));
  const pendingArchive = snapshot.data.samples.filter(sample => sample.reviewDecision === 'Accepted' && !archivedIds.has(sample.id)).length;
  ...
  const renderJobs = (values: JobSummary[]) => values.length === 0
    ? <p className="workspace-jobs__empty">{t(`${copyKey}.workspace.jobs.runningEmpty`)}</p>
  ...
  if (datasetsQuery.isPending || jobsQuery.isPending) return <div className="page-stack workspace-page"><PageHeader title={t(`${copyKey}.workspace.title`)} /><p role="status">{t(`${copyKey}.common.state.loading.body`)}</p></div>;
  ...
  return (
    <div className="page-stack workspace-page">
      ...
      {error ? <section className="generation-feedback" role="alert"><p>{apiErrorMessage(error, locale)}</p><Button variant="secondary" onClick={() => { createMutation.reset(); updateMutation.reset(); void Promise.all([datasetsQuery.refetch(), jobsQuery.refetch()]); }}>{t(`${copyKey}.common.retry`)}</Button></section> : null}
      ...
      <section className="panel workspace-jobs" aria-labelledby="workspace-jobs-title"><div className="section-header"><h2 id="workspace-jobs-title">{t(`${copyKey}.workspace.jobs.title`)}</h2></div><div className="workspace-jobs__grid"><section className="workspace-jobs__group"><h3>{t(`${copyKey}.workspace.jobs.runningTitle`)}</h3>{renderJobs(runningJobs)}</section><section className="workspace-jobs__group"><h3>{t(`${copyKey}.workspace.jobs.failedTitle`)}</h3>{renderJobs(failedJobs)}</section></div></section>
