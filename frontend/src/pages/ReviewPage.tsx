...
export function ReviewPage() {
  ...
  }, [category, datasetId, datasetsById, i18n.language, pendingSamples, protocol, search]);
  ...
  const backToQueue = () => {
    ...
    next.delete('sample');
    setParams(next, { replace: true });
    window.requestAnimationFrame(() => queueRef.current?.focus());
  };

  const returnToSource = () => {
    if (!returnTarget) return;
    const state = location.state as ReviewNavigationState | null;
    if (state?.reviewReturnTo === returnTarget) {
      navigate(-1);
      return;
    }
    ...
    <section className={`page-stack review-page${mobileDetail ? ' review-page--mobile-detail' : ''}`} aria-label={t('review.aria.page')}>
      ...
      {pendingSamples.length === 0 && !selected ? <section className="state-view review-page__state"><h2>{t('review.emptyTitle')}</h2><p>{t('review.emptyBody')}</p></section> : visible.length === 0 && !selected ? <section className="state-view review-page__state"><h2>{t('review.filteredTitle')}</h2><p>{t('review.filteredBody')}</p></section> : (
        <div className={`review-grid${mobileDetail ? ' review-grid--mobile-detail' : ''}`} data-selected-sample={selected?.displayId} data-sample-revision={selected?.revision}>
          <section ref={queueRef} tabIndex={-1} className="panel review-queue" aria-label={t('review.aria.queue')}><div className="section-header"><h2>{t('review.queue')}</h2></div><ul className="review-queue__list">{visible.map(sample => <li key={sample.id} className={sample.id === selected?.id ? 'is-active' : undefined} data-sample-id={sample.displayId}><button type="button" className="review-queue__item" onClick={() => choose(sample)} aria-current={sample.id === selected?.id ? 'true' : undefined}><span className="review-queue__item-main"><strong>{sample.displayId}</strong><span>{sample.category}</span></span><span className="review-queue__dataset">{datasetsById.get(sample.datasetId)?.name ?? sample.datasetId}</span><StatusBadge label={t('status.review.Pending')} kind="neutral" /></button></li>)}</ul></section>
          {selected ? <div className="review-detail">
            <nav className="panel review-detail__navigation" aria-label={t('review.aria.detailNavigation')}>
              <Button variant="quiet" onClick={backToQueue}>{t('review.backToQueue')}</Button>
              {returnTarget ? <Button variant="quiet" onClick={returnToSource}>{t('review.returnToSource')}</Button> : null}
            </nav>
