...
export function ArchivePage() {
  ...
  const samplesQuery = useReviewSampleListQuery({
    ...
    decision: 'All',
    protocol: category === 'All' ? null : category.endsWith('VA') ? 'VA' : 'VT',
    relation: category === 'All' ? null : category.startsWith('A') ? 'Aligned' : 'Conflict',
    direction: null,
    inArchive: true,
    page,
  });
  ...
  const archiveTotalPages = samplesQuery.data?.totalPages ?? 0;
  const listTotal = samplesQuery.data?.total ?? 0;
  ...
  return (
    <div className="page-stack archive-page" aria-label={t('archive.aria.page')}>
      ...
      {datasets.length === 0 ? <section className="panel archive-state"><h2>{t('archive.emptyTitle')}</h2><p>{t('workspaceSettingsStatistics.workspace.datasets.emptyBody')}</p></section> : <>
        <section className="panel archive-toolbar" aria-label={t('archive.aria.toolbar')}><label className="archive-dataset-select"><span>{t('archive.datasetLabel')}</span><select value={dataset?.id ?? ''} onChange={event => selectDataset(Number(event.target.value))}>{datasets.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><div className="archive-toolbar__actions"><Button variant="secondary" busy={previewMutation.isPending} disabled={reviewerId === null} onClick={() => { if (dataset) { setPreviewPage(1); previewMutation.mutate({ datasetId: dataset.id }); } }}>{t('actions.previewSync')}</Button>{archive?.manifestAvailable ? <a className="button button--quiet" href={`/api/archives/${archive.datasetId}/manifest`} download="manifest.jsonl">{t('archive.downloadJsonl')}</a> : null}</div></section>
