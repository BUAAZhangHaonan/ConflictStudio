...
type ApiErrorKind = 'notFound' | 'revision' | 'reviewRevision' | 'noteDraftRevision' | 'reference' | 'archivePreviewStale' | 'gpu' | 'database' | 'renderer' | 'promptEnvelope' | 'promptEmpty' | 'promptJson' | 'promptDuplicateKey' | 'promptSchema' | 'displayName' | 'http' | 'network' | 'invalidInput' | 'modelSwitchConfirmationRequired' | 'unavailable';
...
function kindFor(status: number, code: string): ApiErrorKind {
  ...
  if (code === 'archive_preview_stale') return 'archivePreviewStale';
